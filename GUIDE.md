...
curl -H 'X-Tenant-ID: {tenant_id}' \
  'http://localhost:8081/api/v1/search?q=database+failover+procedure&limit=10'
```
...
```bash
curl -H 'X-Tenant-ID: {tenant_id}' \
  'http://localhost:8081/api/v1/graph/ego?doc_id={doc_id}&hops=2'
```
...
```bash
# 검토 완료 → 게시
curl -X PATCH http://localhost:8080/api/v1/documents/{doc_id}/status \
  -H 'Content-Type: application/json' \
  -H 'X-Tenant-ID: {tenant_id}' \
  -d '{"status": "published"}'
```
...
[스킬 실행 시작]
    ...
    │       GET /api/v1/graph/ego?doc_id={found_doc}&hops=1
    │
...
| 서비스 | 포트 | 역할 |
|--------|------|------|
| Ingest API | `:8080` | 문서 저장, 상태 관리, 운영 정보 |
| Query API | `:8081` | 검색, 피드백, 그래프 탐색 |
| Dashboard | `:3000` | 시각화 (Search/Graph/Curation/Quality/Ops) |
...
```
Ingest API → [Outbox] → NATS JetStream
                            │
                  ┌─────────┼─────────┬─────────────┐
                  ▼         ▼         ▼             ▼
...
```
GET /api/v1/graph/ego?doc_id={id}&hops={1-3}
  → 문서 중심 N-hop 관계 네트워크
```
