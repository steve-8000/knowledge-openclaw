...
import (
	...
	ingesthttp "github.com/openclaw/ki-db/internal/http/ingest"
	opshttp "github.com/openclaw/ki-db/internal/http/ops"
...
func main() {
	...
	ingestHandler := ingesthttp.NewHandler(pool, outbox.NewWriter(), logger)
	r.Route("/api/v1", func(api chi.Router) {
		api.Use(tenancy.Middleware(cfg.Tenancy.TenantHeader))
		ingestHandler.Register(api)
		opshttp.NewHandler(pool).Register(api)
	})
	...
	go func() {
		logger.Info("ingest api listening", "addr", server.Addr)
	...
	case err := <-errCh:
		if !errors.Is(err, http.ErrServerClosed) {
			logger.Error("ingest api failed", "error", err)
			os.Exit(1)
		}
	}
